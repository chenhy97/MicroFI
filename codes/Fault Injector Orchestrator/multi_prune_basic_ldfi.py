...
def evaluator(test_case_name, base_triedHypotheses, total_triedHypotheses, hypothesis, formulas, solutions, blades_ids, recorded_traceid, parent_index, historical_file):
    ...
    if correctness == True or len(hypothesis) == 0:
        ...
        if len(hypotheses) != 0:
            res_solutions,triedHypotheses = evaluateHypotheses(test_case_name, base_triedHypotheses, total_triedHypotheses, hypotheses,formulas,solutions, blades_ids, recorded_traceid,global_index - 1, historical_file)
...
log_path = "./logs/multi_basic_" + datetime.now().strftime("%Y-%m-%d-%H:%M:%S") + "LDFI solve logs"
